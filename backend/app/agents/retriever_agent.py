...
logger = logging.getLogger("pmc_chatbot.agents.retriever")


class RetrieverAgent:
    """
    Specialized Agent for Hybrid Semantic Template Retrieval.
    Combines Dense E5 Vector Cosine Similarity + Lexical BM25 Search using Reciprocal Rank Fusion (RRF).
    """
    ...
    def get_top_candidates(
        ...
        top_k: int = 5
    ) -> List[Tuple[QueryTemplate, float]]:
        """
        ...
        # 3. Reciprocal Rank Fusion (RRF) Computation (k = 60)
        ...
        logger.info(f"RetrieverAgent retrieved top {min(top_k, len(rrf_results))} candidates for query: '{query_text[:50]}...'")
